...
public:
    ...
    void PrintSummary() {
        ...
        if (percentage == 100.0) {
            std::cout << "║  \033[32mSuccess: " << percent_str << "%\033[0m"
                      << std::string(46 - percent_str.length(), ' ') << "    ║\n";
        } else if (percentage >= 80.0) {
            std::cout << "║  \033[33mSuccess: " << percent_str << "%\033[0m"
                      << std::string(46 - percent_str.length(), ' ') << "    ║\n";
        } else {
            std::cout << "║  \033[31mSuccess: " << percent_str << "%\033[0m"
                      << std::string(46 - percent_str.length(), ' ') << "    ║\n";
        }
...
public:
    ...
    ~TestScope() {
        ...
        if (m_Failed == 0) {
            ...
            std::cout << "║  \033[31m[FAIL]\033[0m " << m_Failed << " failed"
    ...
    void Check(bool cond, const char* condStr, const char* msg = "") {
        ...
        if (cond) {
            ...
            m_Failed++;
            m_Results.push_back(
                {std::string(condStr), std::string(msg ? msg : ""), false});
        }
    }
